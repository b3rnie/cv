...
Distributed systems, Erlang/OTP, Scala/Java, embedded systems, low power embedded,
...
\item[] \textit{Engineer}
  ...
  \begin{innerlist}
    \item
      Worked on the core infrastructure using Scala. Major contributions include: developing a scheduling service to
      contact debtors an the optimum time, a rule system to systematically deal with the compliance and regulations in
      debt collection, automatic dispute handling, components to throttle and rate-limit requests to external services.
    \item
...
\item[] \textit{Consulting}
  ...
  \begin{innerlist}
    \item
      Designed, implemented and maintained a batch processing system using Zookeeper, Riak and Erlang/OTP. The system was built
      to scale and be able to handle terrabytes of data each day.
  \end{innerlist}
\end{outerlist}
...
\item[] \textit{Engineer}
  ...
  \begin{innerlist}
    \item
      Architecture, development and maintenance of the services that made up Kivra's system.
  \end{innerlist}
\end{outerlist}
...
\begin{innerlist}
    ...
    \item
      Good knowledge of Scala, Java, Ruby, C, C$+$$+$, Perl
    \item
      Experience with JavaScript, Python, Haskell
\end{innerlist}
...
\begin{innerlist}
    ...
    \item
      ...
      Jxta, NNTP, Irc, BitTorrent and Socks 4/5 as well as other less known protocols.
    \item
      Done web development in PHP, Ruby on Rails, AngularJS, Perl, CGI
\end{innerlist}
